...
window.cargarMiActividad = async function() {
    ...
    try {
        ...
        resultado.data.forEach(int => {
            let icono = int.tipo === 'Llamada' ? '📞' : int.tipo === 'Correo' ? '✉️' : '💬';
            ...
            html += `
                <div style="display: flex; gap: 15px; align-items: flex-start; border-left: 2px solid var(--verde-logo); padding-left: 20px; margin-left: 15px; position: relative;">
                    <div style="position: absolute; left: -16px; top: 0; background: var(--verde-logo); width: 30px; height: 30px; border-radius: 50%; display: flex; justify-content: center; align-items: center; color: white; font-size: 0.9rem;">
                        ${icono}
                    </div>
